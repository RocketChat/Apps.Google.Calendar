...
export class WebhookEndpoint extends ApiEndpoint {
    ...
    public async get(request: IApiRequest,endpoint:IApiEndpointInfo, read:IRead, modify: IModify, http: IHttp,persist: IPersistence): Promise<IApiResponse> {
        ...
        const newresponse = await http.post(url, { params: {'code':`${auth_code}`,'client_id':`${this.Client_id}`,'client_secret':`${this.secret}`,'redirect_uri':'http://localhost:3000/general','grant_type':'authorization_code',}});
